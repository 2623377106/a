...
use think\Request;
use think\Url;
use app\admin\model\brand\SystemAdmin as AdminModel;
...
class SystemAdmin extends AuthController
{
    ...
    public function index()
    {
        ...
        $this->assign('role', \app\admin\model\brand\SystemRole::getRole(bcadd($admin->level, 1, 0)));
    ...
    public function create()
    {
        ...
        $f[] = Form::select('roles', '管理员身份')->setOptions(function () use ($admin) {
            $list = \app\admin\model\brand\SystemRole::getRole(bcadd($admin->level, 1, 0));
            $options = [];
    ...
    public function save(Request $request)
    {
        ...
        foreach ($data['roles'] as $v) {
            $role = \app\admin\model\brand\SystemRole::where('id',$v)->find();
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::select('roles', '管理员身份', explode(',', $admin->roles))->setOptions(function () use ($admin) {
            $list = \app\admin\model\brand\SystemRole::getRole($admin->level);
            $options = [];
            foreach ($list as $id => $roleName) {
                $options[] = ['label' => $roleName, 'value' => $id];
            }
